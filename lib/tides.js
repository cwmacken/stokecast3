...
var exports = module.exports = {
  ...
  getTideLa: function(cb) {
    ...
    request(url, function(err, res, body) {
      ...
      request(imgUrl).pipe(fs.createWriteStream(__dirname + '/../cacheddata/tidedata/sm_tide.png').on('close', function() {
        if (err === null) {
          push.send("Stokecast", "Get LA tide successful");
        } else {
          push.send("Stokecast", "Get LA tide failed");
        }
        cb(err)
      }));
